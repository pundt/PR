...
namespace TestSuite
{
    public class Programmieren
    {
        ...
        /// <summary>
        /// Prüft ob der Typ eine public, Instanz-Eigenschaft mit dem angegebenen Namen und Typ besitzt.
        /// Die Prüfung wird mittels Assertions-Tests durchgeführt.
        /// </summary>
        /// <param name="typ">der Typ der die Eigenschaft beinhalten soll</param>
        /// <param name="feldName">der Name der Eigenschaft die gesucht werden soll</param>
        /// <param name="eigenschaftsTyp">der Typ den die Eigenschaft haben soll</param>
        public static void PrüfeEigenschaft(Type typ, string eigenschaftsName, Type eigenschaftsTyp)
        {
            PropertyInfo propertyInfo = typ.GetProperty(eigenschaftsName, BindingFlags.Public | BindingFlags.Instance);

            if (propertyInfo != null)
            {
                Assert.IsTrue(propertyInfo != null, string.Format("Die Eigenschaft \"{0}\" fehlt!", eigenschaftsName));
                Assert.IsTrue(propertyInfo.PropertyType == eigenschaftsTyp, string.Format("Die Eigenschaft \"{0}\" muss den Datentyp \"{1}\" haben", eigenschaftsName, eigenschaftsTyp));
            }
            else
            {
                Assert.Fail(string.Format("Eigenschaft {0} wurde nicht gefunden!", eigenschaftsName));
            }
        }
        ...
        /// <summary>
        /// Diese Methode prüft ob die Logik in einer Methode den geplanten Rückgabewert hat
        /// </summary>
        /// <typeparam name="T">der Typ für die Prüfungsroutine</typeparam>
        /// <param name="obj">die zu prüfende Instanz</param>
        /// <param name="methodenName">der Name der auszuführenden Methode</param>
        /// <param name="parameter">etwaige Parameter-Werte für die Methode</param>
        /// <param name="predicate">die Regel um festzustellen ob der Rückgabewert korrekt ist.</param>
        public static void PrüfeMethode<T>(object obj, string methodenName, object[] parameter, Predicate<T> predicate)
        {
            ...
            if (methodInfo != null)
            {
                try
                {
                    T rückgabeWert = (T)methodInfo.Invoke(obj, parameter);
                    Assert.IsTrue(predicate.Invoke(rückgabeWert),
                        string.Format("Die Methode \"{0}\" liefert einen falschen Rückgabewert!", methodenName));
                }
                catch (TargetInvocationException ex)
                {
                    throw ex.InnerException;
